...
OUTPUT_FILE = Path("./GOMACTech2025.xlsx")
SOURCE_SHEET_NAME = "GOMACTech 2025"
...
def infer_company_domain(company_name, email):
    ...
    # Prefer company-name-based inference.
    if company_text and "@" not in company_text:
        ...
        # If it already looks like a domain, keep it.
...
def convert():
    ...
    for r in get_source_rows(ws):
        # If the second and third cells are both empty, ignore the row.
        c2 = ws.cell(r, 2)
        c3 = ws.cell(r, 3)
        c2_blank = c2.value is None or str(c2.value).strip() == ""
        c3_blank = c3.value is None or str(c3.value).strip() == ""
        if c2_blank and c3_blank:
            ignored_rows += 1
            continue

        # If the first cell is empty but the second is not, use the second.
        c1 = ws.cell(r, 1)
        effective_first = c1 if (c1.value is not None and str(c1.value).strip() != "") else c2

        if effective_first is None or effective_first.value is None or str(effective_first.value).strip() == "":
            ignored_rows += 1
            continue

        if has_strikethrough(effective_first):
            ignored_rows += 1
            continue
        ...
        # Create Date from Date Added if it is a real date.
        # If it is non-empty but not a date, append it to Notes.
        create_date = ""
        date_added_note = ""
        if date_added_col:
            ...
            if is_date_like(c):
                ...
                text = clean(c.value)
                if text:
                    date_added_note = f"Date Added: {text}"

        # Position from the first column whose label starts with "Verified"
        ...
        company_domain = infer_company_domain(company_name, email_values[0] if email_values else "")
        ...
        for first_name, last_name, email in paired_rows:
            deal_name = f"{company_name}-{first_name}-{last_name}-GOMACTech 2025"
